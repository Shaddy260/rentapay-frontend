...
const FAQ_ITEMS = [
  {
    ...
    audiences: ['landlord', 'tenant', 'admin', 'scout', 'guest'],
    items: [
      {
        q: 'What is RentaPay?',
        a: 'RentaPay is a property management and operations platform built for the Kenyan market. Landlords track properties, units, and tenants in one place, and can bring on property managers or caretakers with their own scoped access; tenants view their balance and account from their phone. Rent payments are one part of this - handled through M-Pesa (STK push, Paybill, or a manual record by the landlord) - alongside the day-to-day work of running a property.',
      },
      {
  ...
  {
    section: 'Scouting for landlords',
    audiences: ['scout'],
    items: [
      {
        q: 'What does a Scout do on RentaPay?',
        a: "As a Scout, you subscribe to one or more counties and get a live feed of vacant units in them - so you can help clients find a place and refer them to the landlord, without needing your own portfolio. You only ever see units in counties you've actively paid for.",
      },
      {
        q: 'How do county subscriptions work?',
        a: 'Each county subscription runs for one year from the date you pay (or from your current expiry, if you renew an already-active one). You can subscribe to as many counties as you like, and each is billed and tracked separately, so one county lapsing doesn\'t affect the others.',
      },
      {
        q: 'How do I find vacant units?',
        a: 'Use "Browse Vacancies" from your sidebar or dashboard. Units are grouped by constituency within each subscribed county, so you can scan just the area you\'re working rather than one long list. You can switch to see occupied or all units too, and filter down to a single county if you\'re subscribed to more than one.',
      },
      {
        q: 'How do I contact a landlord about a unit?',
        a: 'Tap "Message landlord" on any vacancy to open a direct chat with that unit\'s landlord, or call the contact number shown on the listing. A landlord can choose to hide their units from scouts entirely, or block a specific scout - if that happens, their units simply won\'t appear in your feed.',
      },
      {
        q: 'What if I don\'t get the M-Pesa prompt when subscribing?',
        a: 'If the STK push doesn\'t arrive, use "Pay manually" on the subscription screen and enter the M-Pesa transaction code, amount, payer name, and phone exactly as shown on your confirmation SMS. An admin reviews and confirms it, and you\'ll see the outcome on your dashboard.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        a: "RentaPay is built to run your property's day-to-day operations - tenants, units, managers - with the same care as its payment handling. Rent payments run through M-Pesa directly, so RentaPay never holds your money: payments are confirmed automatically by Safaricom's own systems, and every transaction gets a real M-Pesa receipt number you can verify independently.",
      },
      {
