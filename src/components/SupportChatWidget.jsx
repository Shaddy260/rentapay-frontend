...
export default function SupportChatWidget({ token }) {
  ...
  async function handleMenuSelect(key, offerReason, offerCategory) {
    if (key === 'agent') {
      try {
        ...
        const res = await api.escalateSupportToAgent({ category: offerCategory || null, reason: offerReason || 'menu_exhausted' }, token);
    ...
    try {
      const res = await api.selectSupportMenuOption(key, token);
      pushMessage({ from: 'assistant', text: res.reply, menu: res.menu || null });
    } catch {
          ...
          <div className="support-chat-window__list" ref={listRef}>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`support-chat-window__msg support-chat-window__msg--${m.from}`}>
                ...
                {m.offerAgent && !m.tel && (
                  <button type="button" className="support-chat-window__agent-btn" onClick={() => handleMenuSelect('agent', m.offerReason, m.offerCategory)}>
                    Talk to an agent
                  </button>
                )}
                {m.menu && (
                  <div className="support-chat-window__menu">
                    {m.menu.map((opt) => (
                      <button key={opt.key} type="button" onClick={() => handleMenuSelect(opt.key)}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
