...
export default function ChatConversation({ token, role, roleLevel = null, thread, onBack, initialReplyTo = null }) {
  ...
  // DIRECT REQUEST: "long pressing the messages should select one or
  // multiple and allow to delete" - WhatsApp-style selection mode.
  // Bulk-deletes as "delete for me" only (always allowed regardless of
  // sender - see chat.controller.js, scope 'self' has no permission
  // check) by firing the existing single-message delete endpoint once
  // per selected id; no new bulk endpoint needed since that's the same
  // net effect with zero added backend risk.
  ...
  async function handleBulkDelete() {
    ...
    setError('');
    try {
      await Promise.all([...selectedIds].map((id) => api.deleteChatMessage(id, 'self', token)));
      exitSelectMode();
      await load(true);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to delete selected messages.');
    } finally {
  ...
  const load = useCallback(async (silent) => {
    ...
    try {
      const res = await api.listChatMessages(
        { threadType: thread.threadType, landlordId: thread.landlordId, tenantId: thread.tenantId },
        token
      );
  ...
  }, [token, thread.threadType, thread.landlordId, thread.tenantId]);
  ...
  }, [thread.threadType, thread.landlordId, thread.tenantId]);
  ...
  return (
    <div className="chat-conversation">
      <div className="chat-conversation__header">
        {selectMode ? (
          <>
            <button type="button" className="chat-conversation__back" onClick={exitSelectMode} aria-label="Cancel selection">×</button>
            <h4>{selectedIds.size} selected</h4>
            <button
              type="button"
              className="chat-conversation__bulk-delete"
        ...
        {!loading && messages.map((m, i) => {
          ...
          return (
            <React.Fragment key={m.id}>
              ...
              <div
                className={`chat-bubble ${mine ? 'chat-bubble--mine' : ''} ${selectMode && selectedIds.has(m.id) ? 'chat-bubble--selected' : ''}`}
                onMouseDown={() => !deleted && startLongPress(m.id)}
                onMouseUp={cancelLongPress}
                onMouseLeave={cancelLongPress}
                onTouchStart={() => !deleted && startLongPress(m.id)}
                onTouchEnd={cancelLongPress}
                onClick={() => {
                  if (selectMode && !deleted) toggleSelected(m.id);
                }}
              >
                {selectMode && !deleted && (
                  <span className={`chat-bubble__select-check ${selectedIds.has(m.id) ? 'is-checked' : ''}`} aria-hidden="true">
