...
export default function PaymentHistoryPanel({ token, role = 'landlord', propertyId, propertyIdReady = true, canDelete = true, isCaretaker = false }) {
  ...
  // receipt as one zip, for record-keeping.
  ...
  async function handleDownloadAllReceipts() {
    ...
    setError('');
    try {
      await api.downloadAllReceiptsZip(token, { propertyId });
    } catch (err) {
      ...
      <div className="tenant-section__header-row">
        ...
        {payments.length > 0 && (
          <div className="u-flex-row" style={{ gap: 8 }}>
            ...
            <button className="ghost-link" onClick={handleDownloadAllReceipts} disabled={downloadingReceipts}>
              {downloadingReceipts ? 'Preparing…' : 'Download all receipts'}
            </button>
          </div>
        )}
      </div>
