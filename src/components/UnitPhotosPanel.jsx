import { useState } from 'react';
import { api, ApiError } from '../api/client.js';
import PhotoLightbox from './PhotoLightbox.jsx';
import ApplyPhotosToUnitsModal from './ApplyPhotosToUnitsModal.jsx';
import PhotoScrollStrip from './PhotoScrollStrip.jsx';

// FEATURE (direct request: unit photos - "features to improve
// appearance and functionality"). A vacant-unit listing had no photos
// at all, text-only. This gives
// a landlord/manager a simple way to add up to 5 photos per unit
// directly from the unit's own page.
export default function UnitPhotosPanel({ unitId, propertyId, photoUrls = [], token, canEdit = true, onChange }) {
  const [photos, setPhotos] = useState(photoUrls);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState('');
  const [removingUrl, setRemovingUrl] = useState(null);
  const [lightboxIndex, setLightboxIndex] = useState(null);
  const [showApplyModal, setShowApplyModal] = useState(false);
  const [applyNotice, setApplyNotice] = useState('');

  async function handleFilesChosen(e) {
    const files = Array.from(e.target.files || []);
    if (files.length === 0) return;
    setError('');
    setUploading(true);
    try {
      const formData = new FormData();
      files.forEach((f) => formData.append('photos', f));
      const res = await api.uploadUnitPhotos(unitId, formData, token);
      setPhotos(res.photoUrls);
      onChange?.(res.photoUrls);
      // Prompt right after a successful upload, while it's obviously
      // relevant - not a separate button someone has to go looking for.
      setShowApplyModal(true);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to upload photos.');
    } finally {
      setUploading(false);
      e.target.value = ''; // allow re-selecting the same file(s) again later
    }
  }

  async function handleRemove(url) {
    setRemovingUrl(url);
    setError('');
    try {
      const res = await api.removeUnitPhoto(unitId, url, token);
      setPhotos(res.photoUrls);
      onChange?.(res.photoUrls);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to remove photo.');
    } finally {
      setRemovingUrl(null);
    }
  }

  return (
    <section style={{ marginTop: 20 }}>
      <h3 style={{ marginBottom: 8 }}>Photos</h3>
      {photos.length === 0 && !canEdit && <p className="tenant-portal-hint">No photos added yet.</p>}
      {photos.length > 0 && (
        <PhotoScrollStrip className="unit-photos-panel__strip">
          {photos.map((url, i) => (
            <div key={url} className="photo-scroll-strip__item" style={{ position: 'relative' }}>
              <button
                type="button"
                onClick={() => setLightboxIndex(i)}
                aria-label={`View photo ${i + 1} of ${photos.length}`}
                style={{ padding: 0, border: 'none', background: 'none', cursor: 'pointer', display: 'block' }}
              >
                <img src={url} alt="Unit" loading="lazy" decoding="async" style={{ width: 112, height: 112, objectFit: 'cover', borderRadius: 8, display: 'block' }} />
              </button>
              {canEdit && (
                <button
                  type="button"
                  aria-label="Remove this photo"
                  onClick={() => handleRemove(url)}
                  disabled={removingUrl === url}
                  style={{
                    position: 'absolute',
                    top: -6,
                    right: -6,
                    borderRadius: '50%',
                    width: 22,
                    height: 22,
                    lineHeight: '20px',
                    padding: 0,
                    background: '#fff',
                    border: '1px solid #ccc',
                  }}
                >
                  {removingUrl === url ? '…' : '✕'}
                </button>
              )}
            </div>
          ))}
        </PhotoScrollStrip>
      )}
      {canEdit && photos.length < 5 && (
        <label className="form-field__label" style={{ display: 'inline-block' }}>
          <span style={{ display: 'inline-block', marginBottom: 4 }}>
            {uploading ? 'Uploading…' : `Add photo${photos.length > 0 ? 's' : ''} (up to ${5 - photos.length} more)`}
          </span>
          <input type="file" accept="image/jpeg,image/png,image/webp" multiple disabled={uploading} onChange={handleFilesChosen} />
        </label>
      )}
      {canEdit && photos.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <button
            type="button"
            onClick={() => setShowApplyModal(true)}
            style={{ background: 'none', border: 'none', padding: 0, color: 'var(--color-primary, #1f6f5c)', textDecoration: 'underline', cursor: 'pointer', fontSize: '0.85rem' }}
          >
            Use these photos on other units too
          </button>
        </div>
      )}
      {applyNotice && <p className="tenant-portal-hint" style={{ marginTop: 6 }}>{applyNotice}</p>}
      {error && <p className="modal-error">{error}</p>}
      {lightboxIndex !== null && (
        <PhotoLightbox
          photos={photos}
          index={lightboxIndex}
          title="Unit photo"
          onIndexChange={setLightboxIndex}
          onClose={() => setLightboxIndex(null)}
        />
      )}
      {showApplyModal && (
        <ApplyPhotosToUnitsModal
          unitId={unitId}
          propertyId={propertyId}
          token={token}
          onClose={() => setShowApplyModal(false)}
          onApplied={(count) => {
            setShowApplyModal(false);
            setApplyNotice(`Applied these photos to ${count} other unit${count === 1 ? '' : 's'}.`);
          }}
        />
      )}
    </section>
  );
}
