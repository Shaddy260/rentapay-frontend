 ...
 */
export default function DisputesPanel({ token, role = 'landlord', isCaretaker = false }) {
  ...
  function load() {
    // Role Permissions spec (Section 3): Disputed Charges is in the
    // caretaker "no access at all" list - backend 403s this now, so
    // skip the fetch rather than surfacing a failed-request error.
    if (isCaretaker) {
      setLoading(false);
      return;
    }
    ...
    return (
      <section className="statistics-panel">
        ...
        <p className="tenant-portal-hint">
          This section isn't available to caretaker accounts. Contact the landlord or property manager about disputed charges.
        </p>
      </section>
    );
  }
