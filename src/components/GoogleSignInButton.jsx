...
function loadGoogleIdentityScript() {
  ...
  gsiScriptPromise = new Promise((resolve, reject) => {
    ...
    script.onerror = () => {
      // Let the next call try again from scratch instead of being
      // stuck forever on a promise that already rejected once - a
      // transient network blip shouldn't permanently disable the
      // button for the rest of the page's lifetime.
      gsiScriptPromise = null;
      reject(new Error('Failed to load Google Sign-In. Check your internet connection.'));
    };
...
// FIX (Sept 2026 - direct request: "why does sign in with Google
// disappear when there is a network glitch, it should always remain
// persistent"): a transient blip loading Google's script used to mean
// the button's container div just stayed permanently empty - nothing
// ever retried, and nothing was shown in its place, so it looked like
// the feature had vanished rather than hit a one-off network hiccup.
// Two changes: (1) automatically retry a couple of times with a short
// delay, since most real-world "glitches" resolve within a second or
// two on their own; (2) if it still hasn't loaded, show a visible,
// tappable "Try Google Sign-In again" placeholder instead of an empty
// space, so there's always something there and a way to recover
...
export default function GoogleSignInButton({ onCredential, onError, text = 'continue_with' }) {
  ...
  useEffect(() => {
    if (missingClientId) return undefined;
    let cancelled = false;
    let retryTimer = null;
    const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;

    function attemptLoad(attemptNumber) {
      loadGoogleIdentityScript()
        ...
        .catch((err) => {
          ...
          if (attemptNumber < MAX_LOAD_ATTEMPTS) {
            retryTimer = setTimeout(() => attemptLoad(attemptNumber + 1), RETRY_DELAY_MS);
          } else {
            setLoadFailed(true);
  ...
  }, [domId, text, missingClientId, attempt]);
  ...
  if (loadFailed) {
    return (
      <button
        type="button"
        className="google-signin-button google-signin-button--retry"
        style={{ minHeight: 40, minWidth: 320 }}
        onClick={() => {
          setLoadFailed(false);
          setAttempt((n) => n + 1);
        }}
      >
        Couldn't load Google Sign-In - tap to try again
      </button>
    );
  }

  // min-height/min-width reserve the space Google's button will fill
  // once its script loads, so the layout never collapses to nothing
  // while that's in flight - it's always visibly "there," just
  // momentarily empty rather than gone.
  return <div ref={containerRef} id={domId} className="google-signin-button" style={{ minHeight: 40, minWidth: 320 }} />;
}
