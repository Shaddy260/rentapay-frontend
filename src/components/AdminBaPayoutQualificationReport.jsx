            ...
            <>
              <div className="admin-ba-qr__summary">
                ...
                <div className="admin-ba-qr__export-actions">
                  <Button onClick={downloadCsv} disabled={downloadingKey === 'csv'} variant="ghost">
                    {downloadingKey === 'csv' ? 'Downloading…' : 'Download CSV'}
                  </Button>
                  <Button onClick={downloadCombinedPdf} disabled={downloadingKey === 'pdf'}>
                    ...
                    <div className="admin-ba-qr__ba-row" onClick={() => setExpandedBaId(isExpanded ? null : ba.baId)}>
                      <div>
                        ...
                        <div className="admin-ba-qr__ba-stats">
                          {ba.totalLandlordsOnboarded} onboarded · <span className="admin-ba-qr__qualifies">{ba.qualifyingLandlordsWithPayment} qualifying &amp; paid</span> · <span className="admin-ba-qr__not-qualifies">{ba.notQualifyingLandlords} not qualifying</span> · <strong>{fmtKes(ba.totalOwed)}</strong> owed
                        </div>
                      </div>
                      <Button
                      ...
                      <ul className="admin-ba-qr__landlords">
                        {ba.landlords.map((l) => (
                          <li key={l.landlordId} className={`admin-ba-qr__landlord admin-ba-qr__landlord--${l.qualifiesThisCycle ? 'qualifies' : 'not-qualifies'}`}>
                            ...
                            <span className="admin-ba-qr__landlord-status">{l.qualifiesThisCycle ? 'Paid out' : (l.reason || 'Not qualifying')}</span>
                          </li>
                        ))}
                      </ul>
                    )}
