 ...
 */
export default function AdminLoyaltyDiscounts({ token }) {
  ...
  async function grant() {
    ...
    try {
      ...
      const res = await api.bulkGrantLoyaltyDiscount(
        { landlordIds: Array.from(selected), discountPercentage: pct, note: note || undefined, expiryDays: expiryDaysNum },
        token
      );
      setGrantResult(res);
      setDiscountPercent('');
  ...
  async function revoke(landlordId) {
    ...
    try {
      await api.revokeLoyaltyDiscount(landlordId, token);
      loadActive();
      loadCandidates();
      if (historyOpen) loadHistory();
    } catch (err) {
