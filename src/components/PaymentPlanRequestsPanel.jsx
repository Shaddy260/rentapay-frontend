 ...
 */
export default function PaymentPlanRequestsPanel({ token, isCaretaker = false }) {
  ...
  function load() {
    // Role Permissions spec (Section 3): Payment Plan Requests is in
    // the caretaker "no access at all" list - backend 403s this now,
    // so skip the fetch rather than surfacing a failed-request error.
    if (isCaretaker) {
      setLoading(false);
      return;
    }
    ...
    return (
      <section className="statistics-panel">
        ...
        <p className="tenant-portal-hint">
          This section isn't available to caretaker accounts. Contact the landlord or property manager about payment plan requests.
        </p>
      </section>
    );
  }
  ...
  async function handleDecide(requestId, decision) {
    ...
    try {
      ...
      // FEATURE (direct request): once a request is approved/declined it
      // should disappear from the pending view straight away, and the
      // sidebar "Payment Plan Requests" badge count should drop with it
      // instead of continuing to show the old pending count.
  ...
  if (loading && requests.length === 0) return <section className="statistics-panel"><Skeleton rows={4} /></section>;
