...
const FIELD_LABELS = {
  ...
  deposit_amount_paid: 'Deposit amount paid',
};
...
export default function TenantOnboardingPanel({ token, propertyId, propertyName, canAct = true, onConfirmed }) {
  ...
  function startEdit(req) {
    ...
    setEditForm({
      ...
      depositAmountPaid: req.deposit_amount_paid != null ? String(req.deposit_amount_paid) : '',
    });
  }
                ...
                {editingId === req.id ? (
                  <div className="onboarding-request-card__edit">
                    {Object.entries({
                      ...
                      depositAmountPaid: editForm.depositAmountPaid,
                    }).map(([key, value]) => {
                      ...
                      // DIRECT REQUEST: "highlight the deposit field...
                      // should see, and correct if it's wrong" - a
                      // tenant self-reports this, so it gets its own
                      // visual treatment in the edit form too, not
                      // just the summary view below.
                      const isDeposit = key === 'depositAmountPaid';
                      return (
                        <label
                          key={key}
                          className={`onboarding-request-card__field${isDeposit ? ' onboarding-request-card__field--deposit' : ''}`}
                        >
                          <span>{FIELD_LABELS[key.replace(/([A-Z])/g, '_$1').toLowerCase()] || key}</span>
                          <input
                            type={key === 'moveInDate' ? 'date' : isDeposit ? 'number' : 'text'}
                            min={isDeposit ? '0' : undefined}
                            placeholder={isDeposit ? 'None entered' : undefined}
                            value={value || ''}
                            readOnly={isEmail}
                            disabled={isEmail}
                  ...
                  <>
                    <div className="onboarding-request-card__summary">
                      ...
                      {/* DIRECT REQUEST: "in the submitted list also
                          highlight the deposit field so that the
                          landlord or whoever is submitting should see,
                          and correct if it's wrong" - self-reported by
                          the tenant, so it's called out visually
                          rather than blending in with the rest of the
                          summary. */}
                      <span className="onboarding-request-card__deposit">
                        Deposit paid: {req.deposit_amount_paid != null ? `KES ${Number(req.deposit_amount_paid).toLocaleString()}` : 'None entered'}
                      </span>
                    </div>
                    {canAct && (
                      <div className="onboarding-request-card__actions">
