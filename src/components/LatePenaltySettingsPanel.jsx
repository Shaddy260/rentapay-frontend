...
export default function LatePenaltySettingsPanel({ token, isManager, properties }) {
  ...
  return (
    <section className="settings-card late-penalty-panel">
      ...
      <TenantOverrideManager token={token} />
    </section>
  );
}
 ...
 * detail screen), pick the month it applies to, and the reason. A
 * tenant only ever belongs to one apartment, so no apartment picker
 * is needed here.
 */
function TenantOverrideManager({ token }) {
  ...
  async function loadOverrides(id) {
    ...
    if (!id.trim()) return;
    try {
      const res = await api.listLatePenaltyOverrides(id.trim(), token);
      setOverrides(res.overrides || []);
    } catch (err) {
      setLoadError(err instanceof ApiError ? err.message : 'Failed to load overrides for this tenant.');
    }
  }
  ...
  async function removeOverride(id) {
    setBusy(true);
    try {
      await api.removeLatePenaltyOverride(id, token);
      await loadOverrides(tenantId);
    } catch (err) {
