...
/**
 * Per-apartment tenant list with a Download tab (Excel) and an Add to
 * WhatsApp Group tab, plus who-joined / who-left views. Every row is
 * unit-specific to the selected apartment and expands on tap into the
 * same TenantContactCard used elsewhere, showing the tenant's photo
...
export default function TenantListExport({ token, propertyId, propertyName }) {
  ...
  async function handleAddToWhatsAppGroup() {
    ...
    setNotice('');
    try {
      const res = await api.addTenantListToWhatsAppGroup({ propertyId, tenantIds }, token);
      setNotice(res.message);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to add tenants to the WhatsApp group.');
    } finally {
  ...
  return (
    <div className="tenant-list-export">
      ...
      <div className="tenant-list-export__actions">
        ...
        <button
          type="button"
          className="tenant-list-export__action-btn tenant-list-export__action-btn--whatsapp"
          onClick={handleAddToWhatsAppGroup}
          disabled={tenants.length === 0 || addingToGroup || listType === 'left' || listType === 'left_all'}
          title={listType === 'left' || listType === 'left_all' ? 'Only current or newly-joined tenants can be added to a group' : ''}
        >
          {addingToGroup ? 'Adding…' : `📲 Add ${selected.size > 0 ? selected.size : 'all'} to WhatsApp Group`}
        </button>
      </div>
