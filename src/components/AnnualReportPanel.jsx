 ...
 * caretaker needs or should have access to. The annual report and P&L
 * export stay available to caretakers; only the tax summary button and
 * its form are hidden.
 */
export default function AnnualReportPanel({ token, propertyId, isCaretaker = false }) {
  ...
  async function handleFinancialReportCsv() {
    setDownloading('csv');
    setError('');
    try {
      await api.downloadFinancialReportCsv(token, { year, propertyId: effectivePropertyId });
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to generate the financial report.');
    } finally {
      setDownloading('');
    }
  }
  ...
  return (
    <div className="annual-report-panel">
      <div className="annual-report-panel__row">
        ...
        <button className="ghost-link" onClick={handleFinancialReportCsv} disabled={downloading !== ''}>
          {downloading === 'csv' ? 'Preparing…' : `⬇ P&L report (Excel/CSV)${effectivePropertyId ? ' — this property' : ''}`}
        </button>
        {!isCaretaker && (
