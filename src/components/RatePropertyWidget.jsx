 ...
 * existing landlord/manager/caretaker rating widgets - rated by
 * whoever is CURRENTLY a tenant of the property, not tied to any one
 * staff member. Like the others, only the AGGREGATE is ever shown -
 * never a single review tied back to an identifiable tenant, since
 * this same aggregate is also surfaced publicly on the vacant-units
 * listing page (see public.controller.js).
 */
export default function RatePropertyWidget({ token }) {
  ...
  async function submit() {
    ...
    setSubmitError('');
    try {
      const res = await api.rateProperty({ rating: ratingValue, category: 'overall', comment: comment || undefined }, token);
  ...
  // No property linked to this tenant's unit yet - nothing to rate.
  if (reputation && reputation.propertyId === null && reputation.totalRatings === 0 && !showForm) return null;
