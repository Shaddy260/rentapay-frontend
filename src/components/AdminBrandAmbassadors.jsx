 ...
 */
export default function AdminBrandAmbassadors({ token, initialHighlightId }) {
  ...
  // Global-search deep-link: jumping here from a BA search result
  // (see AdminDashboard.jsx's handleGlobalSearchSelect) drops straight
  // into the roster, across every status, with that BA's row
  // highlighted and scrolled into view - same idea as the tenant/unit
  // and manager/caretaker deep-links elsewhere in the admin panel.
  const [highlightId, setHighlightId] = useState(initialHighlightId || '');

  useEffect(() => {
    if (!initialHighlightId) return;
    setView('roster');
    setRosterStatus(''); // "all" - the highlighted BA might be in any status
    setHighlightId(initialHighlightId);
  }, [initialHighlightId]);

  useEffect(() => {
    if (!highlightId || view !== 'roster' || !roster) return;
    const el = document.getElementById(`ba-roster-item-${highlightId}`);
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }, [highlightId, view, roster]);
          ...
          {!roster ? (
            ...
            <ul className="admin-ba__list">
              {roster.map((b) => (
                <li
                  key={b.id}
                  id={`ba-roster-item-${b.id}`}
                  className={`admin-ba__item${b.id === highlightId ? ' admin-ba__item--highlight' : ''}`}
                >
