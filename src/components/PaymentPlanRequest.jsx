...
export default function PaymentPlanRequest({ token, totalDue }) {
  ...
  // FEATURE (spec item 4): long-press an installment row to select it,
  // then a delete option appears - same 450ms-hold pattern already
  // used for message selection in ChatConversation.jsx.
  ...
  function startLongPress(idx) {
    ...
    longPressTimerRef.current = setTimeout(() => setSelectedIndex(idx), 450);
  }
  ...
  async function confirmDeleteInstallment() {
    ...
    setDeleteError('');
    try {
      await api.deletePaymentPlanInstallment(current.id, selectedIndex, token);
      setSelectedIndex(null);
      load();
    } catch (err) {
      setDeleteError(err instanceof ApiError ? err.message : 'Failed to remove that installment.');
    } finally {
      setDeleting(false);
    }
  }
    ...
    <div className="tenant-section" style={{ marginTop: 16 }}>
      ...
      {current?.status === 'approved' && (
        <div className="paybill-pending" style={{ borderColor: '#2E7D32' }}>
          ...
          {/* FIX (spec item 4): fixed max-height + internal scroll,
              same pattern as the landlord dashboard's Chronically Late
              Units table - this used to grow unbounded with however
              many installments the plan had. */}
          <div className="paybill-pending__details payment-plan__scroll">
            {(current.installments || []).map((inst, i) => (
              <div
                key={i}
                className={`payment-plan__row${selectedIndex === i ? ' payment-plan__row--selected' : ''}`}
      ...
      <ConfirmDialog
        ...
        message={selectedIndex !== null && current?.installments?.[selectedIndex] ? `Remove installment ${selectedIndex + 1} (KES ${Number(current.installments[selectedIndex].amount).toLocaleString()} by ${new Date(current.installments[selectedIndex].dueDate).toLocaleDateString('en-GB')}) from your payment plan?` : ''}
