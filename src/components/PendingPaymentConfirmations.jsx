...
export default function PendingPaymentConfirmations({ token, canConfirmReject = true, propertyId }) {
  ...
  async function handleContactTenant(record) {
    ...
    if (!phone) {
      setError('No phone number on file for this tenant.');
      return;
    }
    try {
      await navigator.clipboard.writeText(phone);
      setNotice(`Copied ${phone} to clipboard.`);
    } catch {
              ...
              <div key={record.id} className={`ppc-card ${record.resubmission_of ? 'ppc-card--resubmitted' : ''}`}>
                ...
                <div className="ppc-card__details">
                  ...
                  {record.units?.unit_payment_code && (
                    <div><span>Account no. / unit code</span><span>{record.units.unit_payment_code}</span></div>
                  )}
                ...
                {record.status === 'pending' && record.duplicate_of && (
                ...
                {record.status === 'pending' && !record.duplicate_of && canConfirmReject && (
                  <div className="ppc-card__actions">
