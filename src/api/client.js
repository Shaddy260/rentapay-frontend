...
export const api = {
  ...
  getPendingPaymentConfirmations: (status, token) => request(`/payments/pending-confirmations${status ? `?status=${status}` : ''}`, { token }),
  ...
  listAdminSqlRows: (table, { limit, offset } = {}, token) => {
    const params = new URLSearchParams();
