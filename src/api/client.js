...
// FIX (Sept 2026 - direct request: sessions/OTP dying within seconds,
// "you have been logged out - unusual activity"): the in-flight guard
// above only dedupes calls within ONE browser tab - it's a plain
// module-level variable, so a second tab (or a re-opened tab from
// earlier in the same browser) has its own completely separate copy
// with no idea the first tab is refreshing at all. With RentaPay open
// in more than one tab - extremely normal for anyone tabbing between
// the dashboard and something else - both tabs' access tokens expire
// at the same moment (they were issued together), so both
// independently call /auth/refresh with the SAME refresh token at
// nearly the same time. The first call wins and rotates it; the
// second arrives half a second later with a refresh token that's
// already been used once, which the backend's reuse-detection
// correctly identifies as looking exactly like a stolen token being
// replayed - so it revokes the ENTIRE token family as a safety
// measure. That kills the session in every tab, including the one
// that "won," which is exactly the symptom this was causing.
//
// navigator.locks gives an actual mutex across every tab of the same
// origin (not just this one JS context), so only one tab anywhere
// ever performs the real network call at a time - every other tab
// waits for the lock, then re-reads localStorage first: if the
// waiting tab sees the refresh token already changed underneath it,
// another tab has already refreshed successfully, so it just uses
// that new token instead of wastefully (and dangerously) replaying
// the old one. Supported in every current browser; falls back to the
// previous single-tab-only behavior on anything ancient enough not
// to have it, which is strictly the pre-existing behavior, never
// worse.
async function refreshAccessToken() {
  if (refreshInFlight) return refreshInFlight;

  const rtBeforeLock = getStoredRefreshToken();

  const doRefresh = async () => {
    // Another tab may have already refreshed while we were waiting
    // for the lock - if the stored refresh token moved on from the
    // one we captured before acquiring it, use the token that's
    // already there instead of replaying a now-dead one.
    if (getStoredRefreshToken() !== rtBeforeLock) {
  ...
  refreshInFlight = (async () => {
    if (typeof navigator !== 'undefined' && navigator.locks && navigator.locks.request) {
      return navigator.locks.request('rentapay_refresh_lock', doRefresh);
    }
    // No Web Locks support - same single-tab-only behavior as before.
    return doRefresh();
  })();
...
async function request(path, { method = 'GET', body, token, queueable, queueDescription, keepalive, _isRetry = false } = {}) {
  ...
  try {
  try {
    // FIX (screenshot: "Please wait..." stuck forever on a degraded
    // connection): fetch() had no timeout at all - on a connection
    // that's bad enough to hang rather than cleanly fail, the promise
    // just never resolves or rejects, so the try/catch/finally below
    // never runs and the button never comes back from its loading
    // state. A 25s AbortController timeout guarantees this always
    // eventually settles one way or the other, with the same
    // network-error message and retry path as an outright failure.
...
async function requestMultipart(path, { method = 'POST', formData, token, _isRetry = false } = {}) {
  ...
  beginTrackedRequest();
  try {
    // Same fix as request() above, but a longer window (60s) since
    // file uploads legitimately take longer than a JSON round trip on
    // a slow connection - don't want to abort an upload that's simply
    // still in progress.
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000);
    try {
      response = await fetch(`${BASE_URL}${path}`, { method, headers, body: formData, signal: controller.signal });
    } finally {
      clearTimeout(timeoutId);
    }
  } catch (networkErr) {
...
export const api = {
  ...
  // Mandatory 2FA (TOTP) second step - same endpoint for first-time
  // QR setup confirmation and every regular login after that.
  generalManagerVerifyTotp: (payload) => request('/auth/manager-account/verify-totp', { method: 'POST', body: payload }),
  // FEATURE (direct request): "general manager dont have a way to
  // reset their password ... add it and also add it from the login
  // screen." Same shape as the shared forgot-password pair above,
  // just scoped to the general_managers table server-side.
  generalManagerForgotPassword: (payload) => request('/auth/manager-account/forgot-password', { method: 'POST', body: payload }),
  generalManagerResetPassword: (payload) => request('/auth/manager-account/reset-password', { method: 'POST', body: payload }),
  ...
  // First-time admin 2FA QR setup confirmation (mandatory - replaces
  // the old emailed-OTP flow entirely).
  confirmAdminTotpSetup: (payload) => request('/auth/admin/confirm-totp-setup', { method: 'POST', body: payload }),
  adminForgotPassword: (payload) => request('/auth/admin/forgot-password', { method: 'POST', body: payload }),
  ...
  // OPTIONAL self-service 2FA toggle (landlord/tenant/manager/brand_ambassador)
  get2faStatus: (token) => request('/auth/2fa/status', { token }),
  start2faEnable: (token) => request('/auth/2fa/enable/start', { method: 'POST', token }),
  confirm2faEnable: (payload, token) => request('/auth/2fa/enable/confirm', { method: 'POST', body: payload, token }),
  disable2fa: (payload, token) => request('/auth/2fa/disable', { method: 'POST', body: payload, token }),
  ...
  // Live Paybill/Till a landlord/manager/caretaker pays their
  // subscription to - any logged-in payer can read this (view only).
  getPlatformPaymentDetails: (token) => request('/settings/platform-payment', { token }),
  ...
  // payments - strictly admin only, both to view (with history) and
  // to change.
  getPlatformPaymentSettings: (token) => request('/admin/settings/platform-payment', { token }),
  updatePlatformPaymentSettings: (payload, token) => request('/admin/settings/platform-payment', { method: 'PATCH', body: payload, token }),
  ...
  // Late payment penalty (Settings -> Finances) - one settings row per
  // landlord account, plus per-tenant/per-period overrides.
  getLatePenaltySettings: (propertyId, token) => request(`/late-penalty/properties/${propertyId}/settings`, { token }),
  updateLatePenaltySettings: (propertyId, payload, token) => request(`/late-penalty/properties/${propertyId}/settings`, { method: 'PATCH', body: payload, token }),
  previewLatePenalty: (payload, token) => request('/late-penalty/settings/preview', { method: 'POST', body: payload, token }),
  // propertyId here is the apartment currently selected in the picker
  // above this manager (LatePenaltySettingsPanel) - passed through so
  // the backend can reject a tenant that belongs to a different
  // apartment, instead of silently letting an override apply to the
  // wrong one. See checkTenantMatchesSelectedApartment on the backend.
  listLatePenaltyOverrides: (tenantId, propertyId, token) => request(`/late-penalty/tenants/${tenantId}/overrides${propertyId ? `?propertyId=${propertyId}` : ''}`, { token }),
  createLatePenaltyOverride: (tenantId, payload, token) => request(`/late-penalty/tenants/${tenantId}/overrides`, { method: 'POST', body: payload, token }),
  removeLatePenaltyOverride: (overrideId, propertyId, token) => request(`/late-penalty/overrides/${overrideId}${propertyId ? `?propertyId=${propertyId}` : ''}`, { method: 'DELETE', token }),
  ...
  confirmManualSubscriptionPayment: (id, token, extra = {}) => request(`/admin/landlord-manual-subscription-payments/${id}/confirm`, { method: 'POST', body: extra, token }),
  rejectManualSubscriptionPayment: (id, reason, token, extra = {}) => request(`/admin/landlord-manual-subscription-payments/${id}/reject`, { method: 'POST', body: { reason, ...extra }, token }),
  deleteManualSubscriptionPayment: (id, token, extra = {}) => request(`/admin/landlord-manual-subscription-payments/${id}`, { method: 'DELETE', body: extra, token }),
  ...
  // REMOVED (direct request): initiateRentSTKPush / checkRentPaymentStatus /
  // initiateUtilityStkPush called endpoints that would have run rent/
  // utility money through RentaPay's own Paybill. RentaPay only
  // collects its own subscription fee - rent/utilities are paid
  // directly to the landlord's own Paybill/Till/phone and self-reported
  // via submitPaybillTransaction below.
  ...
  listUtilityInvoicesForTenant: (token, tenantId) => request(`/payments/utility-manual/open-invoices-for-tenant?tenantId=${encodeURIComponent(tenantId)}`, { token }),
  ...
  getMyLatestPaybillConfirmation: (token, targetInvoiceId) =>
    request(`/payments/my-latest-confirmation${targetInvoiceId ? `?targetInvoiceId=${targetInvoiceId}` : ''}`, { token }),
  // payload: { transactionCode, amountPaid, mpesaPayerName, mpesaSmsTimestamp? }
  ...
  disableDarajaCredentials: (token) => request('/landlord/daraja-credentials/disable', { method: 'POST', token }),
  // Tenant-facing push + self-heal poll (mirrors checkSubscriptionPaymentStatus's pattern):
  tenantStkPush: (payload, token) => request('/payments/tenant-stk-push', { method: 'POST', body: payload, token }),
  checkTenantStkStatus: (paymentId, token) => request(`/payments/landlord-stk-status/${paymentId}`, { token }),
  ...
  // FIX (direct request: "landlord confirms the payment and it does
  // not go to confirmed, it just keeps appearing there... this made
  // the payment be confirmed multiple times, so billing issue"):
  // these were `queueable: true`. On a flaky connection, a failed
  // confirm/reject silently got queued in IndexedDB and the app
  // returned a fake `{queued:true}` success - the toast said
  // "confirmed" but the real backend record was untouched, so it
  // reappeared as pending on the next refetch. That looked like the
  // confirm "didn't work", so a landlord would naturally tap Confirm
  // again - queuing a SECOND action for the same record. Once back
  // online, both queued actions replayed against the backend, firing
  // the confirm side-effects (balance update, billing) twice for one
  // real payment. Queueing is fine for things like a chat message or
  // maintenance request where a duplicate replay is harmless - it's
  // wrong for anything that moves money. This now behaves like any
  // other non-queueable write: a failed request throws immediately
  // and visibly, instead of silently queuing and lying about success.
  ...
  // admin-portal page + incoming-items banner.
  listGmPendingActions: (token) => request('/admin/general-managers/pending-reviews', { token }),
  getGmPendingActionCount: (token) => request('/admin/general-managers/pending-reviews/count', { token }),
  reviewGmPendingAction: (logId, decision, token) =>
    request(`/admin/general-managers/pending-reviews/${logId}/review`, { method: 'POST', body: { decision }, token }),
  bulkReviewGmPendingActions: ({ logIds, all } = {}, decision, token) =>
    request('/admin/general-managers/pending-reviews/bulk-review', { method: 'POST', body: { decision, logIds, all }, token }),
  ...
  // Phase 2: async exports (queue a job, poll status, download the
  // finished file's signed URL). See routes/exportJobs.routes.js.
  createAnnualReportJob: (params, token) => request('/export-jobs/annual-report/pdf', { method: 'POST', body: params, token }),
  createTaxSummaryJob: (params, token) => request('/export-jobs/tax-summary/pdf', { method: 'POST', body: params, token }),
  createFinancialCsvJob: (params, token) => request('/export-jobs/financial-report/csv', { method: 'POST', body: params, token }),
  createReceiptsZipJob: (params, token) => request('/export-jobs/receipts-zip', { method: 'POST', body: params, token }),
  createDataExportJob: (params, token) => request('/export-jobs/data-export', { method: 'POST', body: params, token }),
  getExportJobStatus: (id, token) => request(`/export-jobs/status/${id}`, { token }),
  getExportJobDownload: (id, token) => request(`/export-jobs/download/${id}`, { token }),
  ...
  selectSupportSubmenuOption: (category, key, token) => request('/support-chat/submenu-select', { method: 'POST', body: { category, key }, token }),
  ...
  // Force-update gate - public, no token, see ForceUpdateGate.jsx.
  getAppVersionCheck: () => request('/app/version-check'),
  getAppVersionConfig: (token) => request('/app/version-config', { token }),
  updateAppVersionConfig: (payload, token) => request('/app/version-config', { method: 'PUT', body: payload, token }),
  ...
  listPendingUtilityReviews: (token, propertyId) => request(`/utility-submetering/readings/pending-review${propertyId ? `?propertyId=${encodeURIComponent(propertyId)}` : ''}`, { token }),
