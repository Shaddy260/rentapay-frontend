...
export const api = {
  ...
  markNotificationRead: (notificationId, token) => request(`/notifications/${notificationId}/read`, { method: 'POST', token }),
  ...
  submitManualPropertyPayment: (payload, token) => request('/properties/purchase/manual-payment', { method: 'POST', body: payload, token }),
  listManualPropertyPayments: (status, token) => request(`/admin/property-purchase-manual-payments?status=${encodeURIComponent(status || 'pending')}`, { token }),
  confirmManualPropertyPayment: (id, token) => request(`/admin/property-purchase-manual-payments/${id}/confirm`, { method: 'POST', token }),
  rejectManualPropertyPayment: (id, reason, token) => request(`/admin/property-purchase-manual-payments/${id}/reject`, { method: 'POST', body: { reason }, token }),
