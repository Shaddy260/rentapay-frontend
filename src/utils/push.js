 ...
 * The auto-fix runs on its own the first time someone opens the
 * installed app (see the standalone-detection logic there) - this
 * exists only for edge cases where that didn't catch it (e.g. someone
 * cleared just this one flag, or a browser where display-mode
 * detection is unreliable). Tears down the current subscription and
 * creates a fresh one from wherever it's called.
 */
export async function resubscribePush(token) {
  ...
  try {
    ...
    if (existing) {
      ...
      try {
        await api.unsubscribePush(oldEndpoint, token);
      } catch {
        // Non-fatal - the old row is harmless clutter if this fails;
        // sendPushToRecipient already self-prunes dead endpoints.
      }
    }
    ...
    console.warn('[push] resubscribePush failed:', err.message);
