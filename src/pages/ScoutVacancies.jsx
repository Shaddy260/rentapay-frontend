...
export default function ScoutVacancies() {
  ...
  // Optional "I have a specific lead for this unit" details, captured
  // per-unit right before sharing - lets the referral represent an
  // actual prospective tenant (name shows up on the scout's own
  // pipeline view) instead of always being an anonymous "someone might
  // rent this." Entirely optional; sharing works exactly as before if
  // left blank.
  const [prospectDrafts, setProspectDrafts] = useState({}); // { [unitId]: { open, name, phone } }
  // FEATURE (direct request: unit photos were a single non-clickable
  // thumbnail - "clickable and expands upon clicking"). Which unit's
  // gallery is open, and at what photo index, so PhotoLightbox can be
  // rendered once at the bottom and reused for every card.
  const [gallery, setGallery] = useState(null); // { unit, index }
  ...
  function shareUnit(unit) {
    ...
    const draft = prospectDrafts[unit.id];
    const prospect = draft?.name?.trim() || draft?.phone?.trim()
      ? { prospectName: draft.name?.trim() || undefined, prospectPhone: draft.phone?.trim() || undefined }
      : undefined;
    api
      .referScoutUnit(unit.id, token, prospect)
      .then(() => {
        ...
        // Photos: a wa.me link can't attach real image files, but
        // including each photo's URL as its own line lets WhatsApp
        // preview/unfurl them and lets the recipient tap through - the
        // closest a plain share link can get to "share with the pics."
        if (Array.isArray(unit.photoUrls) && unit.photoUrls.length > 0) {
          lines.push('', 'Photos:', ...unit.photoUrls);
        }
  ...
  function load() {
    ...
    setError('');
    api
      .getScoutVacancies('vacant', token)
      .then(setData)
  ...
  return (
    <div className="add-tenant-page">
      ...
      {activeCounties.length === 0 && !loading ? (
        ...
        <>
          <p className="add-tenant-subtitle">
            Showing vacant units in counties you're subscribed to: {activeCounties.join(', ') || '—'}
          </p>
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              message={
                searchNeedle
                  ? `No vacant units match "${areaSearch}". Try a different area or clear the search.`
                  : "Nothing vacant in your subscribed counties at the moment - check back soon, or try subscribing to another county from your scout settings."
              }
              actionLabel={searchNeedle ? 'Clear search' : undefined}
              onAction={searchNeedle ? () => setAreaSearch('') : undefined}
              compact
            />
          ) : (
                    ...
                    <div style={{ display: 'grid', gap: 8, padding: 12 }}>
                      ...
                      >
                        {/* FEATURE (direct request: unit photos for
                            ...
                            Clickable - opens the full gallery via
                            PhotoLightbox, with a "+N" badge when there
                            are more photos than the one shown. */}
                        {u.photoUrls?.[0] ? (
                          <button
                            type="button"
                            onClick={() => setGallery({ unit: u, index: 0 })}
                            aria-label={`View ${u.photoUrls.length} photo${u.photoUrls.length === 1 ? '' : 's'} of ${u.unitName}`}
                            style={{
                              position: 'relative',
                              width: 72,
                              height: 72,
                              ...
                              {shareState[u.id] !== 'shared' && (
                                <>
                                  <button
                                    ...
                                    onClick={() =>
                                      setProspectDrafts((d) => ({ ...d, [u.id]: { ...d[u.id], open: !d[u.id]?.open } }))
                                    }
                                  >
                                    {prospectDrafts[u.id]?.open ? '– Hide lead details' : '+ Got a lead for this? (optional)'}
                                  </button>
                                  {prospectDrafts[u.id]?.open && (
                                    <div style={{ display: 'flex', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                                      <input
                                        type="text"
                                        placeholder="Prospect's name"
                                        value={prospectDrafts[u.id]?.name || ''}
                                        onChange={(e) =>
                                          setProspectDrafts((d) => ({ ...d, [u.id]: { ...d[u.id], name: e.target.value } }))
                                        }
                                        style={{ padding: 4, fontSize: '0.85em', flex: 1, minWidth: 100 }}
                                      />
                                      <input
                                        type="text"
                                        placeholder="Prospect's phone"
                                        value={prospectDrafts[u.id]?.phone || ''}
                                        onChange={(e) =>
                                          setProspectDrafts((d) => ({ ...d, [u.id]: { ...d[u.id], phone: e.target.value } }))
                                        }
                                        style={{ padding: 4, fontSize: '0.85em', flex: 1, minWidth: 100 }}
                                      />
                                    </div>
      ...
      {gallery && (
        <PhotoLightbox
          photos={gallery.unit.photoUrls || []}
          index={gallery.index}
          title={gallery.unit.unitName}
