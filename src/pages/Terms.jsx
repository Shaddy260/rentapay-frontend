  ...
  return (
    <div className="legal-page">
      <div className="legal-page__card">
        ...
        <p>
          By default, a tenant pays rent using M Pesa, whether through a Paybill number, a Buy
          Goods Till number, or a direct Send Money transfer, sent directly to the landlord's own
          M Pesa Paybill, Till number, or personal phone number. This payment never passes through
          a RentaPay controlled account. Once the tenant has made the payment outside the app, the
          tenant then submits proof of that payment within RentaPay, consisting of the M Pesa
          transaction code, the amount paid, the payer name and phone number shown on the M Pesa
          confirmation message, and the time shown on that message. A landlord, property manager,
          or caretaker with access to that tenant's unit then reviews this submission and either
          confirms it as received or rejects it. Section 4.1b below describes an optional,
          landlord-enabled alternative to this manual flow.
        </p>
        <p>
          RentaPay does not hold, take custody of, guarantee, insure, or exercise any control over
          rent funds at any point in this process, in either the manual flow described above or
          the automatic flow described in Section 4.1b. RentaPay records what has been reported by
          a tenant and reviewed by a landlord, manager, or caretaker, or, where automatic
          collection is enabled, what has been reported directly by Safaricom. A payment marked
          confirmed within RentaPay reflects that landlord's, manager's, or caretaker's own
          decision that they personally received the funds, or, for an automatically confirmed
          payment, Safaricom's own confirmation of the transaction. It is not a guarantee made by
          RentaPay, and RentaPay is not liable if a landlord, manager, or caretaker confirms a
          payment they did not actually receive, or fails to confirm a payment they did receive,
          whether that happens by honest mistake or in bad faith. Reviewing and confirming a
          submitted payment under the manual flow is a step performed by a human being on the
          landlord's side and is not instantaneous. It may take anywhere from a few minutes to a
          few hours, and in some cases longer, depending on how quickly the landlord, manager, or
          caretaker responds. A payment confirmation, once made, can still be challenged through
          the in app dispute process described in Section 8 of these Terms, but resolving what
          actually happened with the underlying rent obligation remains a matter between the
        ...
        <h3>4.1b Optional Automatic Rent Collection (Landlord-Owned Till/Paybill)</h3>
        <p>
          A landlord who holds their own Safaricom Business Paybill or Till number, and their own
          Safaricom Daraja API credentials for it, may connect that Paybill or Till to RentaPay.
          This feature is off by default, opt-in, and may be enabled or disabled by the landlord
          at any time; disabling it has no effect on any payment already confirmed. Once enabled
          and verified, a tenant paying rent from within the app is sent an M Pesa STK push prompt
          directly to their phone, initiated using the landlord's own stored credentials against
          the landlord's own Paybill or Till, and the payment is confirmed automatically once
          Safaricom reports it, without a landlord, manager, or caretaker manually reviewing that
          individual payment. As with the manual flow, RentaPay does not hold, take custody of, or
          otherwise control the funds at any point; RentaPay's role is limited to initiating the
          payment request and recording the outcome Safaricom reports. The landlord is solely
          responsible for the accuracy, confidentiality, and security of the Daraja credentials
          they provide, and for ensuring their own Paybill or Till and Daraja application remain in
          good standing with Safaricom; RentaPay is not liable for a payment that fails, is
          delayed, or is misdirected because of an error in those credentials, a change made to
          them outside RentaPay, or an outage or policy change on Safaricom's part. If an automatic
          payment attempt fails for any reason, the tenant is shown the manual payment instructions
          described in Section 4.1 as a fallback.
        </p>
        ...
        <p>
          ...
          and do not create rights or obligations beyond what the rest of these Terms already
          state.
        </p>
        ...
        <p>
          ...
          falls due on the date recorded for their unit. The tenant opens their M Pesa menu on
          their own phone and sends that amount to their landlord's own Paybill number, Till
          number, or personal phone number, exactly as they normally would outside of RentaPay.
          Safaricom processes that transfer as an ordinary M Pesa transaction between the tenant
          and the landlord, with RentaPay having no part in it at all. Once the tenant receives
          their own M Pesa confirmation message, they open RentaPay, go to their unit's payment
          screen, and enter the transaction code, the amount, the payer name, the payer phone
          number, and the time shown on that message. This creates a submission with a pending
          status. The landlord, or whichever manager or caretaker has access to that unit,
          receives a notification about the new submission, reviews it against their own M Pesa
          statement, and either confirms it, which updates the tenant's balance, or rejects it,
          for example because the amount does not match or the transaction code cannot be found,
          which prompts the tenant to correct and resubmit. At every step of this walkthrough, the
          actual money moved directly between tenant and landlord through Safaricom's own systems,
          and RentaPay only ever recorded what the two of them reported and confirmed to each
          other.
        </p>
        <p>
          <strong>Automatic collection (where the landlord has enabled it under Section 4.1b).</strong>{' '}
          The same rent amount falls due, and the tenant opens RentaPay and taps "Pay Rent." Because
          ...
          landlord, manager, or caretaker review step in this path. If the prompt fails to arrive
          or go through for any reason, the tenant is shown the manual payment instructions instead
          and the first walkthrough above applies.
        </p>
        ...
        <p>
          What happens if my landlord does not confirm my payment. Where your landlord has not
          enabled automatic collection, confirmation is a manual step performed by a human being,
          so it can take a few minutes or a few hours; if it is taking longer than that, contact
          your landlord, manager, or caretaker directly, and if the issue cannot be resolved
          between you, use the in app dispute tool described in Section 9. Where your landlord has
          enabled automatic collection under Section 4.1b, there is no manual confirmation step to
          wait on, since Safaricom reports the payment back automatically.
        </p>
        <p>
