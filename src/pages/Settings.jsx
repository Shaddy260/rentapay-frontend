...
import { resubscribePush } from '../utils/push.js';
...
export default function Settings() {
  ...
  const [notificationFixResult, setNotificationFixResult] = useState('');

  async function handleFixNotifications() {
    setFixingNotifications(true);
    setNotificationFixResult('');
    const result = await resubscribePush(token);
    setFixingNotifications(false);
    if (result.ok) {
      setNotificationFixResult('Done — notifications on this device are now set up fresh.');
    } else if (result.reason === 'permission-denied') {
      setNotificationFixResult('Notifications are blocked for this app in your device settings — enable them there first, then try again.');
    } else {
      setNotificationFixResult('Could not refresh notifications on this device — please try again.');
    }
  }
            ...
            <div className="settings-payment-form__grid">
              ...
              <div className="form-field">
                <label className="form-field__label">
                  Still seeing "Chrome" instead of RentaPay on notifications?
                  <InfoTip text={'This normally fixes itself the first time you open the installed RentaPay app after using it in a browser. If it hasn\'t after that, tap this once from inside the app.'} />
                </label>
                <button type="button" className="ghost-link" onClick={handleFixNotifications} disabled={fixingNotifications}>
                  {fixingNotifications ? 'Fixing…' : 'Fix notifications on this device'}
                </button>
                {notificationFixResult && <p className="form-field__hint">{notificationFixResult}</p>}
              </div>
            </div>
            <div className="settings-manager-row__actions">
