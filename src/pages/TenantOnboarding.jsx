...
export default function TenantOnboarding() {
  ...
  const [trialUnitLimit, setTrialUnitLimit] = useState(null);
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        setUnits(res.units || []);
        setTrialUnitLimit(res.trialUnitLimit ?? null);
      })
              ...
              <div className="tenant-onboarding-unit-grid">
                {visibleUnits.map((u) => (
                  <button
                    type="button"
                    key={u.id}
                    className={`tenant-onboarding-unit-btn${u.locked ? ' tenant-onboarding-unit-btn--locked' : ''}`}
                    onClick={() => { if (!u.locked) chooseUnit(u); }}
                    disabled={u.locked}
                    title={u.locked ? `Available once the landlord subscribes. Only the first ${trialUnitLimit} units are open right now.` : undefined}
                  >
                    <strong>{u.unit_name}</strong>
                    {u.unit_type && <span>{u.unit_type}</span>}
                    {u.locked && <span className="tenant-onboarding-unit-btn__locked-tag">Not available yet</span>}
                  </button>
                ))}
              </div>
            )}
