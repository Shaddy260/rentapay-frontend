...
export default function AdminDashboard() {
  ...
  async function handleReplyToHelpRequest(h) {
    ...
    try {
      ...
      // Not a real chat_messages row (help_requests is a separate
      // table), so this is a purely visual quote - id stays null,
      // which sendMessage already treats as "no reply_to" and just
      // sends a normal top-level message. The point is the OTHER
      // person sees exactly what they wrote, right above the admin's
      // reply, so it's obvious what's being answered.
      setReplyToHelpRequest({
        id: null,
  ...
  const [helpCategory, setHelpCategory] = useState('all'); // 'all' | 'tenant' | 'landlord' | 'manager' | 'caretaker' | 'scout' | 'guest'
    ...
    <div className="admin-page">
      <PortalSidebar
        ...
        items={[
          ...
          { key: 'scout-manual-payments', label: 'Scout Manual Payments', icon: '🧭', onClick: () => setActiveTab('scout-manual-payments') },
            ...
            >
              <div className="form-field">
                ...
                <select value={broadcastTargetGroup} onChange={(e) => setBroadcastTargetGroup(e.target.value)}>
                  <option value="all">Everyone (all landlords, managers, caretakers, tenants, and scouts)</option>
      ...
      <main className="admin-main">
        ...
        <nav className="admin-tabs">
          ...
          <button className={activeTab === 'scouts' ? 'is-active' : ''} onClick={() => setActiveTab('scouts')}>Scouts</button>
          <button className={activeTab === 'scout-manual-payments' ? 'is-active' : ''} onClick={() => setActiveTab('scout-manual-payments')}>Scout Manual Payments</button>
        ...
        {activeTab === 'overview' && (
          <>
            <section className="admin-metrics">
              ...
              <button
                type="button"
                className="admin-metric-card admin-metric-card--clickable"
                onClick={() => setActiveTab('scout-manual-payments')}
              >
                <span className="admin-metric-card__label">Total Scouts</span>
                <span className="admin-metric-card__value">{metrics.scouts?.totalScouts ?? 0}</span>
                <span className="admin-metric-card__sub">
                  {metrics.scouts?.activeScouts ?? 0} active · {metrics.scouts?.expiredOnlyScouts ?? 0} expired-only
                </span>
                <span className="admin-metric-card__hint">Review payments →</span>
              </button>
              <button
                type="button"
                className="admin-metric-card admin-metric-card--clickable admin-metric-card--good"
                onClick={() => setActiveTab('scout-manual-payments')}
              >
                <span className="admin-metric-card__label">Scout revenue this month</span>
                <span className="admin-metric-card__value">KES {Number(metrics.scouts?.revenueThisMonth || 0).toLocaleString()}</span>
                <span className="admin-metric-card__sub">{metrics.scouts?.activeCountySubscriptions ?? 0} active county subscriptions</span>
                <span className="admin-metric-card__hint">Review payments →</span>
              </button>
            </section>
        ...
        {activeTab === 'help' && (
          <section className="admin-section">
            ...
            <div className="ppc-status-tabs" style={{ marginBottom: 16 }}>
              {['all', 'tenant', 'landlord', 'manager', 'caretaker', 'scout', 'guest'].map((c) => (
                <button
                  key={c}
                  type="button"
          ...
          <section className="admin-section">
            ...
            <div className="admin-messages-layout">
              <div className="admin-messages-layout__list">
                <ChatThreadList
                  token={token}
                  onSelect={(t) => { setSelectedThread(t); setReplyToHelpRequest(null); }}
                  selectedKey={selectedThread ? `${selectedThread.threadType}:${selectedThread.landlordId || ''}:${selectedThread.tenantId || ''}` : null}
                />
              </div>
              <div className="admin-messages-layout__conversation">
                {selectedThread ? (
                  <ChatConversation token={token} role="admin" thread={selectedThread} initialReplyTo={replyToHelpRequest} />
                ) : (
