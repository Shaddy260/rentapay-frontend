...
export default function AdminDashboard() {
  ...
  function load() {
    ...
    setLoading(true);
    Promise.all([api.getAdminDashboard(token), api.listAllLandlords(token), api.getActivityLog(token), api.getLockdownStatus(token)])
      .then(([metricsRes, landlordsRes, activityRes, lockdownRes]) => {
      ...
      <main className="admin-main">
        ...
        {activeTab === 'help' && (
          <section className="admin-section">
            ...
            {!helpLoading && helpRequests.length === 0 && <p className="admin-section__hint">No {helpFilter !== 'all' ? helpFilter : ''} help requests.</p>}
                ...
                <tbody>
                  {helpRequests.map((h) => (
                    <tr key={h.id}>
                      ...
                      <td>{h.name} <span className="admin-table__estate-name">{h.requester_type}</span></td>
                      <td>{h.phone || '—'}</td>
