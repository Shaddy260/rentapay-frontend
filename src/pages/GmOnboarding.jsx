 ...
 * FIX: submission now lands in an admin pending-approval queue,
 * same as BA onboarding - it no longer creates an active account
 * right away, since the link itself isn't tied to one specific
 * person and anyone with it could otherwise self-activate.
 */
export default function GmOnboarding() {
  ...
  async function handleSubmit(e) {
    ...
    // BUG FIX (direct request): catch an unrecognizable phone number
    // here, with the same check the backend will apply, instead of
    // only finding out after a round trip.
    if (!isValidPhone(form.phone)) {
      setSubmitError('Enter a valid Kenyan phone number (e.g. 07XXXXXXXX or 2547XXXXXXXX).');
      return;
    }
    setSubmitting(true);
    setSubmitError('');
    try {
      ...
      setDoneMessage(res.message || 'Your details have been submitted and are pending admin review. You will receive your login details by email once approved.');
      setStep('done');
    } catch (err) {
      if (err instanceof ApiError && err.raw?.linkExpired) {
        setExpiredMessage(err.message);
