...
// NOTE: 70 (not the blueprint's stated 150) per direct instruction.
const BASE_RATE = 70;
...
const STEPS = [
  ...
  { key: 'verify', title: 'Verify phone', subtitle: 'Enter OTP code' },
...
export default function RegisterFlow() {
  ...
  const [property, setProperty] = useState({
    estateName: '', location: '', county: '', description: '',
  ...
  // -----------------------------------------------------------------
  // STEP 2 -> 3 : "I've paid" - THE FIX for "signed up, paid, but
  // never received the OTP": this used to just advance to the OTP
  // step on click, trusting that Safaricom's callback had already
  // reached the backend and activated the account. If that callback
  // was late, unreachable (DARAJA_CALLBACK_URL wrong/no tunnel
  // running), or simply never fires in a dev setup, the account was
  // still 'pending' with no OTP ever generated - and the OTP screen
  // this used to jump straight to would fail with a confusing
  // "Invalid OTP" no matter what the person typed.
  //
  // Now this actually polls the backend (which self-heals by asking
  // Safaricom directly if needed - see checkSubscriptionPaymentStatus
  // in payment.controller.js) until the payment is confirmed one way
  // or the other, and only advances once activation genuinely
  // happened - so a fresh OTP is guaranteed to exist by the time the
  // person lands on that screen.
  // -----------------------------------------------------------------
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        if (res.status === 'failed') {
          ...
          setPaymentPollError(
            res.reason
              ? `Payment was not completed: ${res.reason}. You can try again from the previous step.`
              : 'Payment was not completed (cancelled or timed out). You can try again from the previous step.'
          );
          return;
        }
  ...
  async function handleVerifyOtp(e) {
    ...
    setLoading(true);
    try {
      await api.verifyOTP({ accountType: 'landlord', accountId: landlordId, otp: otp.join('') });
      ...
      // wizard step after OTP had no way to authenticate its requests.
      ...
      try {
        ...
        // Don't hard-fail the whole OTP step over this - worst case the
        // wizard's later steps silently skip their backend calls (each
        // checks for a token before calling) and the person can just
        // log in normally afterward to finish setup.
        console.warn('Auto-login after OTP failed, continuing without a token:', loginErr.message);
      }
  ...
  function handleOtpDigitChange(i, value) {
    ...
    if (token) {
      ...
      try {
        await api.updatePropertyDetails(
          { estateName: property.estateName, location: property.location, county: property.county, description: property.description },
          token
        );
  ...
  async function handleUnitsSubmit() {
    ...
    setLoading(true);
    try {
      // Create each unit that doesn't already have a real backend id
      // yet (re-running this after going Back and Forward shouldn't
      // re-create units that already exist).
      const updatedUnits = await Promise.all(
        units.map(async (u) => {
          if (u.id) return u; // already created
          ...
          return { ...u, id: res.unit.id, code: res.unit.unit_payment_code };
        })
      );
      setUnits(updatedUnits);
      setStepIndex(6);
    } catch (err) {
      setError(`Could not save units: ${err.message}`);
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="register-page__panel">
          ...
          {stepIndex === 2 && (
            <>
              ...
              <p className="register-page__intro">
                We sent a 6-digit code to {form.phone}. It expires in 3 days.
              </p>
              <form onSubmit={handleVerifyOtp}>
                ...
                <Button type="submit" variant="primary" loading={loading}>Verify and continue</Button>
              </form>
            </>
          )}
          ...
          {stepIndex === 3 && (
            <>
              <span className="success-badge">✓ Account verified</span>
              ...
              <form onSubmit={handlePropertySubmit}>
                ...
                <div className="register-page__form-grid">
                  ...
                  <div className="form-field">
                    ...
                    <input id="county" required value={property.county} onChange={(e) => setProperty((p) => ({ ...p, county: e.target.value }))} placeholder="Nairobi" />
                  </div>
                </div>
                <div className="form-field">
          ...
          {stepIndex === 6 && (
            <>
              ...
              <div className="register-page__actions">
                <Button type="button" variant="ghost" onClick={() => setStepIndex(5)}>Back</Button>
