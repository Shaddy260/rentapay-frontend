...
import { fieldErrorsFromApi, isValidPhone } from '../utils/validators.js';
...
// Reads ?as=landlord|manager directly from the URL, outside of React
// state, so it's available before the very first render (loading the
// persisted wizard progress below needs to know it immediately).
function readAccountLabelFromUrl() {
...
export default function RegisterFlow() {
  ...
  const currentAccountLabel = readAccountLabelFromUrl();
  // BUG FIX (direct request: "starts signing up as a landlord... goes
  // back and starts signing up as a property manager using the same
  // device... takes him to the last page he was signing up as a
  // different role"): persisted progress was keyed by ONE shared
  // localStorage entry with no record of which role it belonged to,
  // so switching roles on the same device silently resumed the other
  // role's half-finished session, form data and all. Persisted
  // progress now records the accountLabel it was saved under (see the
  // persistProgress() call below), and is only honored here when it
  // matches the role this exact URL is for - a mismatch means this is
  // a genuinely different signup attempt, so the stale one is dropped
  // and this starts clean from step 0, without touching whatever the
  // other role's in-progress attempt was doing.
  let persisted = loadPersistedProgress();
  if (persisted && (persisted.accountLabel || 'landlord') !== currentAccountLabel) {
    persisted = null;
  ...
  const [trialDeniedNotice, setTrialDeniedNotice] = useState('');
  ...
  }, [accountLabel, stepIndex, landlordId, checkoutRequestId, amountDue, paymentInitiated, defaultPropertyId, isTrial, trialUnitLimit, form.fullName, form.phone, form.email, form.unitsCount, form.periodMonths]);
  ...
  async function doHandleSubmitDetails(e) {
    setError('');
    // BUG FIX (direct request, checked repeatedly - "07... rejected,
    // 254... accepted, for a manager signup"): this field had NO
    // client-side check at all before, so a number that was in fact
    // fine would only ever get flagged after a full round trip to
    // registerLandlord, or - worse - a number the backend genuinely
    // can't parse would surface as a raw 400 instead of a clear
    // inline message. Uses the exact same normalizer the backend
    // does (utils/phone.js), for both the landlord and property-
    // manager paths - this wizard is shared by both (see accountLabel
    // above), so there is no manager-only code path to have drifted
    // in the first place; this check covers both identically.
    ...
    try {
      ...
      // BUG FIX (direct request: "the email is new... this is a bug"):
      // don't silently drop someone onto the payment step with no
      // explanation of why they didn't get the trial they expected.
      // The backend now says exactly which field (phone/email/both)
      // already has a trial on record - surface that plainly instead
      // of leaving it to look like the trial feature simply isn't
      // working.
      if (res.trialAlreadyUsed) {
        const reasonText =
          res.trialDeniedReason === 'phone'
            ? 'the phone number you used'
            : res.trialDeniedReason === 'email'
            ? 'the email address you used'
            : 'the phone number and/or email you used';
        setTrialDeniedNotice(
          `You won't get a new free trial this time - ${reasonText} already had one before. You can still continue by paying for a plan below.`
        );
      }
        ...
        <div className="register-page__panel">
            ...
            <>
              ...
              <form onSubmit={handleSubmitDetails} className="register-form">
                ...
                <div className="register-form-section">
                  ...
                  {/* DIRECT REQUEST: the live price preview banner
                      that used to sit here has been removed. Nearly
                      every new signup is trial eligible and won't be
                      charged anything for 7 days, so showing a
                      running "Total due today" figure at this step
                      was misleading. Once the 7 days are up, the
                      landlord sees a clear subscribe banner on their
                      dashboard instead, and only then, once they
                      actually subscribe, do they see the units and
                      price they are subscribing for. */}
                </div>
          ...
          {stepIndex === 1 && (
            <div className="mpesa-pending">
              ...
              {trialDeniedNotice && (
                <div className="api-info-banner" role="status">{trialDeniedNotice}</div>
              )}
