...
const BA_STATS_CACHE_KEY = 'rentapay_ba_stats_cache';
...
function useBaStats(token) {
  const [stats, setStats] = useState(() => readPageCache(BA_STATS_CACHE_KEY)?.stats || null);
  // FIX (direct request - see pageCache.js): this hook is mounted
  // independently by BOTH the Dashboard cards/trend chart AND the
  // Stats section - switching tabs unmounts one and mounts the
  // other, so each used to start from loading=true/stats=null every
  // single time, flashing a skeleton for data that was just fetched
  // seconds earlier by the OTHER section. Seeding from a shared cache
  // means whichever one mounts next already has real numbers on
  // screen while it quietly refetches behind them.
  const [loading, setLoading] = useState(() => !readPageCache(BA_STATS_CACHE_KEY));
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        if (!cancelled) {
          setStats(res);
          writePageCache(BA_STATS_CACHE_KEY, { stats: res });
        }
      })
      .catch((err) => {
...
export default function BaPortal() {
  ...
  // Same cache-and-seed treatment as Dashboard.jsx/TenantPortal.jsx -
  // profile gates the whole portal, so seeding it from the last
  // successful load means a refresh (or remount) has real content
  // immediately instead of blanking to the skeleton every time.
  const baProfileCache = (() => {
    try {
      const raw = localStorage.getItem('rentapay_ba_portal_cache');
      return raw ? JSON.parse(raw) : null;
    } catch {
      return null;
    }
  })();
  const [profile, setProfile] = useState(() => baProfileCache || null);
  const [loading, setLoading] = useState(() => !baProfileCache);
  const [error, setError] = useState('');
  ...
  const load = useCallback(() => {
    ...
    api
      ...
      .then((res) => {
        setProfile(res);
        try {
          localStorage.setItem('rentapay_ba_portal_cache', JSON.stringify(res));
        } catch {
          // non-fatal
        }
      })
