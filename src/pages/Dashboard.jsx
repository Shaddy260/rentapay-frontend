    ...
    <div className="dashboard-page">
      <PortalSidebar
        ...
        items={[
          ...
          { key: 'disputes', label: 'Disputed Charges', icon: '🚩', badge: disputesBadge, onClick: () => setActiveView('disputes') },
          { key: 'payment-plans', label: 'Payment Plan Requests', icon: '📋', badge: planRequestsBadge, onClick: () => setActiveView('payment-plans') },
          ...
          { key: 'community', label: 'Community Board', icon: '🏘️', onClick: () => setActiveView('community') },
      ...
      {activeView === 'due-dates' ? (
        ...
        <main className="dashboard-main">
          <PaymentHistoryPanel token={token} role={role} propertyId={activePropertyId} propertyIdReady={!loading} canDelete={!isCaretaker} />
        </main>
      ) : activeView === 'disputes' ? (
        <DisputesPanel token={token} role={role} />
      ) : activeView === 'payment-plans' ? (
        <PaymentPlanRequestsPanel token={token} />
                ...
                return (
                  <Link to={`/units/${unit.id}`} className={`unit-card ${isOverdue ? 'unit-card--overdue' : ''}`} key={unit.id}>
                    ...
                    <span className="unit-card__tenant">
                      {activeTenant ? (
                        <>
                          <TenantContactCard tenant={{ ...activeTenant, unit_name: unit.unit_name }} size={24} token={token} canRate />
