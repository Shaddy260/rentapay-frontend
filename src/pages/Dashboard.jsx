  ...
  function load(propertyId) {
    ...
    Promise.all([api.getDashboard(token, propertyId), api.listUnits(token, propertyId)])
      .then(([dashboardData, unitsData]) => {
        ...
        const resolvedPropertyId = dashboardData.activePropertyId || null;
  ...
  return (
    <div className="dashboard-page">
      ...
      {subscriptionExpired && (
        <div className="subscription-gate">
          <div className="subscription-gate__card">
            <span className="subscription-gate__icon">⚠️</span>
            <h2>{sub.scopedToPropertyId ? 'This apartment\u2019s subscription has ended' : 'Your RentaPay subscription has ended'}</h2>
            <p>
              {isManager
                ? "The landlord's RentaPay subscription has ended. Contact them to renew it - your access is scoped to the landlord's account and will resume once it's active again."
                : sub.scopedToPropertyId
                  ? "Renew this apartment's subscription to keep managing it. Your other apartments (and their tenants' portals) keep working as normal in the meantime."
                  : "Renew your subscription to continue managing your property with RentaPay. Your tenants' portals keep working as normal in the meantime - this only affects your own dashboard access."}
            </p>
            {!isManager && <Button variant="primary" onClick={() => navigate('/subscription')}>Renew subscription</Button>}
            {/* FIX (direct request): "while he receives that pop up
                subscription window, he should still be able to shift
                to the other his apartments and access them" - this
                used to be a hard, unclosable full-screen block with no
                way out except renewing. Each apartment can carry its
                own independent subscription now, so switching to a
                still-active one should actually work, not just show
                the same gate again. */}
            {properties.length > 1 && (
              <div className="subscription-gate__switch">
                <label htmlFor="subscription-gate-switch">Or switch to another apartment:</label>
                <select
                  id="subscription-gate-switch"
                  value=""
      ...
      {activeView === 'statistics' ? (
        ...
        <main className="dashboard-main">
          <PendingPaymentConfirmations token={token} canConfirmReject={!isCaretaker} />
        </main>
      ...
      <main className="dashboard-main">
        ...
        <section className="quick-actions">
          {!isCaretaker && <Link to="/units/new" className="quick-action-btn">+ Add unit</Link>}
          <Link to="/settings" className="quick-action-btn">Settings</Link>
          <button className="quick-action-btn" onClick={handleBulkRemind}>Send bulk reminder</button>
