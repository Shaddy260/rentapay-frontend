...
export default function Messages() {
  ...
  // bottom-nav icons visibly disappearing/reappearing on navigation).
  // Root cause: every page is a separate lazy-loaded chunk, and
  // BottomNav lives inside each page rather than as a persistent
  // layout - so going Home -> Messages -> Home unmounts the whole
  // screen (nav bar included) and briefly shows App.jsx's blank
  // full-screen "Loading…" Suspense fallback while the next chunk
  // downloads. Warming that chunk up front (as soon as Messages
  // mounts, since Home is the one place every user taps right back to)
  // means it's already resolved by the time they tap - React swaps
  // straight to the mounted page with no fallback in between.
  useEffect(() => {
