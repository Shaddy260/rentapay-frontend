...
import { REGISTER_SETUP_PATH } from '../utils/finalizeLogin.js';
...
export default function RegisterRoleGate() {
  ...
  // REINFORCEMENT (direct request: "no code editing in the future
  // should ever break this again"): an independent, second safety net
  // on top of the fix in finalizeLogin.js/Login.jsx. If someone with a
  // valid session and an unfinished landlord setup ever lands here,
  // for any reason, present or future, they are sent straight into
  // the real wizard instead of being shown the "who are you" choice
  // again, which reads to them as being told to start completely
  // over. This runs regardless of how they arrived here.
  useEffect(() => {
    if (typeof localStorage === 'undefined') return;
    const hasSession = !!localStorage.getItem('rentapay_token');
    const isLandlord = localStorage.getItem('rentapay_role') === 'landlord';
    const setupIncomplete = localStorage.getItem('rentapay_resume_setup') === 'true';
    if (hasSession && isLandlord && setupIncomplete) {
      navigate(REGISTER_SETUP_PATH, { replace: true });
    }
