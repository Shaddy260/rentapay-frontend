...
import { useHelpContacts } from '../components/HelpButton.jsx';
...
export default function PublicListings() {
  ...
  const { helpEmail, helpCall, helpWhatsapp } = useHelpContacts();
  ...
  return (
    <div className="public-listings">
      ...
      <div className="public-listings__fraud-banner" role="note">
        <span className="public-listings__fraud-banner-icon" aria-hidden="true">🛡️</span>
        <div>
          <strong>Stay safe while browsing.</strong>
          Never send money before viewing a unit in person, and be cautious of anyone asking for payment upfront to "hold" a unit.
          {' '}If you suspect someone is impersonating a landlord, caretaker, or agent, or if you believe you have been conned, please report it right away by emailing{' '}
          <a href={`mailto:${helpEmail}`}>{helpEmail}</a>, messaging us on{' '}
          <a href={`https://wa.me/${String(helpWhatsapp).replace(/[^\d]/g, '')}`} target="_blank" rel="noopener noreferrer">WhatsApp</a>, or calling{' '}
          <a href={`tel:${helpCall}`}>{helpCall}</a>. Reporting helps us take action against people trying to defraud tenants.
        </div>
      </div>
