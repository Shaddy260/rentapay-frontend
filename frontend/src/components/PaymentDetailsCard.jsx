...
export default function PaymentDetailsCard({ amount, note }) {
  ...
  useEffect(() => {
    api.getPlatformPaymentDetails(token).then(setDetails).catch((err) => {
      // PRODUCTION ERROR HANDLING FIX: this used to swallow the error
      // entirely, which meant a failed fetch left every field showing
      // "…" with no indication anything was wrong - risking someone
      // trying to pay to a blank/placeholder account number. Now shown
      // as an explicit error state instead of a silent placeholder.
      console.warn('[PaymentDetailsCard] failed to load payment details:', err);
      setLoadFailed(true);
    });
  }, [token]);
  ...
  if (loadFailed) {
    return (
      <div className="payment-details-card payment-details-card--error">
        Couldn't load payment details right now. Please refresh, or contact support@rentapay.co.ke if this keeps happening.
      </div>
    );
  }
