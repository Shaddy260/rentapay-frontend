...
export default function AdminBaRewardsDashboard({ token }) {
  // PRODUCTION ERROR HANDLING FIX: this component used window.alert() to
  // surface a failed PDF download, which is inconsistent with the rest
  // of the app's toast.error() convention (see PendingPaymentConfirmations.jsx,
  // TenantOnboardingPanel.jsx, etc.) - a native browser alert() blocks
  // the whole tab and looks broken next to everything else here.
            ...
            {(history || []).map((r) => (
              <li key={r.id} className="admin-ba-rewards__history-item">
                ...
                <Button
                  ...
                  data-download-fx
                  onClick={() =>
                    api.downloadBaRewardPdf(r.batch_id, token).catch((err) => toast.error(err instanceof ApiError ? err.message : 'Failed to download PDF.'))
                  }
                >
                  Download reward PDF
                </Button>
        ...
        <div className="ba-reward-confirm">
          ...
          <div className="ba-reward-confirm__done-actions">
            <Button
              variant="ghost"
              data-download-fx
              onClick={() => api.downloadBaRewardPdf(result.batch.id, token).catch((err) => toast.error(err instanceof ApiError ? err.message : 'Failed to download PDF.'))}
            >
              Download PDF now
            </Button>
