...
export default function DueDatesCalendar({ token, refreshKey }) {
  const [data, setData] = useState(null);
  const [error, setError] = useState('');
  const navigate = useNavigate();

  // FIX (direct request): a bulk due-date change made from the
  // Dashboard's modal used to leave this calendar showing the old
  // grouping until a full page reload, since this component fetches
  // its own data once on mount and had no way to know a change had
  // just been applied elsewhere - a landlord would save a bulk change
  // and immediately doubt whether it actually went through. The
  // Dashboard now bumps refreshKey after any due-date change (single
  // or bulk) so this refetches right away.
  useEffect(() => {
  ...
  if (!data) return <Skeleton variant="card" count={3} />;
