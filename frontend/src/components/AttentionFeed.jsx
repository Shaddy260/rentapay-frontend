...
export default function AttentionFeed({ token, onOpenTenant, onOpenPendingPayments }) {
  ...
  useEffect(() => {
    if (!token) return;
    api.getAttentionFeed(token).then(setFeed).catch((err) => {
      // Non-fatal: the feed just doesn't render (see the guard below).
      // Logged so a broken feed endpoint doesn't go unnoticed.
      console.warn('[AttentionFeed] failed to load feed:', err);
    });
  }, [token]);
  ...
  // Tenant risk flag (Section 8) - surfaced above ordinary overdue
  // items, in its own critical tone, since 2+ consecutive missed
  // rent cycles is a materially bigger problem than a single late
  // payment and shouldn't blend into that list.
  feed.riskFlaggedTenants?.slice(0, 5).forEach((t) => {
    items.push({
      key: `risk-${t.tenantId}`,
      tone: 'critical',
      text: `⚠ ${t.tenantName} (${t.unitName}) has missed ${t.missedCycles}+ consecutive rent payments - KES ${t.amountDue.toLocaleString()} owed`,
      onClick: () => onOpenTenant?.(t.tenantId, t.unitId),
    });
  });
  if (feed.riskFlaggedTenants?.length > 5) {
    items.push({ key: 'risk-more', tone: 'critical', text: `+${feed.riskFlaggedTenants.length - 5} more tenant(s) flagged as high risk` });
  }
