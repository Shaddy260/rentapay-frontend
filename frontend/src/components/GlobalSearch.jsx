...
export default function GlobalSearch({ token }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      api.globalSearch(query, token).then((res) => { setResults(res); setOpen(true); }).catch((err) => {
        // Non-fatal: the search dropdown just stays closed/empty. Still
        // worth a console trace instead of a bare swallow so a broken
        // search endpoint doesn't go unnoticed in production.
