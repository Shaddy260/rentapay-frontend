...
// FIX (Sept 2026): third-party requests this app doesn't control -
// Cloudflare's analytics beacon, Google's CSP-violation reporting
// endpoint - occasionally get blocked by a CSP directive, an ad
// blocker, or a flaky connection. Before, that rejection propagated
// straight back through respondWith() as an uncaught promise
// rejection, showing up as a scary "TypeError: Failed to fetch" at
// sw.js in the console for something that was never RentaPay's own
// request failing. Catching it here keeps this handler a true no-op
// passthrough (still never caches or blocks anything real) while
// stopping noise from requests this app has no control over anyway.
// FIX (Sept 2026 - direct request: profile photo upload just sits on
// "uploading" forever with no error): this handler used to intercept
// and re-pipe EVERY request, including file uploads - a POST carrying
// a multipart FormData body with the actual image bytes. Passing a
// large binary request body back through fetch() a second time inside
// a service worker is a known source of silent hangs in some browser/
// version combinations: the promise never resolves or rejects, so the
// upload UI just sits on its loading state forever with nothing to
// catch and show as an error. There is no caching upside to
// intercepting a POST/PUT/PATCH/DELETE anyway (nothing here ever
// caches responses), so those now skip this handler entirely and go
// straight to the network exactly as if no service worker existed -
// only GET requests (page loads, the installability check this
// handler exists for) are still passed through respondWith().
self.addEventListener('fetch', (event) => {
  if (event.request.method !== 'GET') return;
  event.respondWith(fetch(event.request).catch(() => new Response(null, { status: 599 })));
});
